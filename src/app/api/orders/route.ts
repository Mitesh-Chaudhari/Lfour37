...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      brandId = brand.id
      locationId = await getDefaultOnlineLocationId(brand.id)
    } catch {
      // Migration 046 not applied — order still works without brand columns.
    }
    ...
    if (brandId) {
      orderPayload.brand_id = brandId
      orderPayload.company_id = YADEVI_COMPANY_ID
      orderPayload.location_id = locationId
      orderPayload.sales_channel = 'online'
    }
